...
class Client:
    def __init__(self, client_id):
        ...
        self.channel.queue_declare(queue=self.response_queue_name, exclusive=True)

        while True:
            self.send_request()

            # Устанавливаем callback-функцию для обработки ответов
            self.channel.basic_consume(queue=self.response_queue_name, on_message_callback=self.callback, auto_ack=True)
    ...
    def send_request(self):
        ...
        self.channel.basic_publish(exchange='direct_exchange', routing_key='requests_queue', body=request_data)
